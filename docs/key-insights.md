...
- **`RUSTUP_TOOLCHAIN: stable` is set in every workflow.** It keeps CI on stable. Device recipes that need the `esp` toolchain (e.g., `check-idf`, Xtensa builds) use `cargo +esp` explicitly; there is no repo-wide `rust-toolchain.toml` because host gates must not require the ESP toolchain. If device builds dominate the workflow (rare), add a `rust-toolchain.toml` then, but keep host recipes explicit with `--target` and device recipes explicit with `+esp` / `+nightly`.
