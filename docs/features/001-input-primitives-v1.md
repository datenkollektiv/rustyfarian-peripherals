...
|                                                                          `AnalogValue` normalizes raw ADC counts to `0..=u16::MAX` with normalized deadbanding | Reusable across potentiometers, sliders, joysticks, and analog sensors; keeps ADC setup in hardware examples       | Potentiometer-specific pure type / raw ADC pass-through only                           |
...
- [x] Tests passing (72 default / 95 with `--features hal`)
