...
|                                                `AnalogCalibration` observes raw extrema; `AnalogValue` normalizes raw ADC counts to `0..=u16::MAX` with deadbanding | Reusable across potentiometers, sliders, joysticks, and analog sensors; keeps ADC setup in hardware examples       | Potentiometer-specific pure type / raw ADC pass-through only                           |
...
- All decode/timing/normalization logic stays host-testable in `tamer` (sans-io boundary).
...
- [x] Analog implementation (`tamer::analog`) plus first potentiometer examples (`hal_c3_poti`, `idf_c3_poti`)
- [x] Tests passing (74 default / 97 with `--features hal`)
...
- 2026-06-24 — added `tamer::analog` for ADC range normalization and deadbanded analog movement, plus `hal_c3_poti` / `idf_c3_poti` as the first basic analog examples.
- 2026-06-25 — added `AnalogCalibration` and taught both potentiometer examples to observe a short startup sweep before normalizing raw ADC values.
