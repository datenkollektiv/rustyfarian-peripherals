#!/usr/bin/env bash
# xtensa-toolchain.sh — shared helper: add xtensa-esp-elf-gcc to PATH if needed.
...
setup_xtensa_toolchain() {
    if ! command -v xtensa-esp32-elf-gcc >/dev/null 2>&1; then
        ...
        if [ -n "$xtensa_bin" ]; then
            ...
            printf 'Source your ESP toolchain environment export script (e.g. ". ~/export-esp.sh"; name/path vary by installation).\n' >&2
            return 1
        fi
    fi
}
