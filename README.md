...
pins, debounced switches, rotary encoders, and button events — for ESP32 and
`no_std` embedded Rust projects. No application code, just reusable, composable
crates.
...
graph TD
    ...
    periph["rustyfarian-peripherals<br/>pins · debounce · rotary · buttons<br/>(this repo)"]
    app --> net
    app --> leds
    app --> pwr
    app --> periph
...
| Repo                                                                             | Role                                                              |
|:---------------------------------------------------------------------------------|:------------------------------------------------------------------|
| **rustyfarian-peripherals** (this repo)                                          | Input peripherals: pins, debounce, rotary encoders, button events |
...
- **Hardware-independent core:** debounce, rotary quadrature decoding, and
  button-event timing live in [`tamer`](crates/tamer) — plain `no_std` Rust with
  no hardware dependency, always compiled, fully testable on the host.
...
|:----------------------------------------------------------------------------|:----------------------|:---------|:----------------------------------------------------------------------------------------------------------------------------|
| [`tamer`](crates/tamer)                                                     | Pure / host-buildable | `no_std` | Debounce, rotary decoding, and button-event logic behind traits, with `Noop*` mocks. No hardware dependency. **(skeleton)** |
...
> **Status: skeleton.** The workspace structure, tooling, and CI are in place;
> the input primitives themselves are added downstream-driven (see
> [ROADMAP.md](./docs/ROADMAP.md)). Every crate builds on the host today.
